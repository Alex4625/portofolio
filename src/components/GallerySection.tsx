    ...
    <section id="gallery" className="py-16 md:py-24 bg-background border-t border-border">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
          {data.map((item, index) => (
            <ScrollReveal key={item.id} delay={(index % 3) * 0.1} width="w-full">
              <div 
                className="relative group break-inside-avoid overflow-hidden rounded-none shadow-sm cursor-zoom-in bg-muted mb-6"
              >
                <img
                  src={item.imageUrl}
                  alt={item.caption}
